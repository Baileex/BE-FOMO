...
class EventsModel(db.Model):
    ...
    def json(self):
        return {'id': self.id, 'name': self.name, 'location': self.location, 'business_id': self.business_id, 'description': self.description, 'event_type': self.event_type, 'date': self.date, 'time': self.time, 'min_age': self.min_age, 'cost': self.cost, 'url': self.url  }
